...
> 当用户要求分析某只股票/ETF/港美A股投资价值时使用本 skill。

默认由当前 Codex agent 主导执行：数据收集 → 研究分析 → 多空观点对照 → 决策报告。
...
> Agent 定义（`bull.md`、`bear.md`）在 `.claude/agents/` 目录中，不会自动注册到 Codex；如需角色分工，请将角色说明作为参考材料读取。
...
单条消息并行派遣 5 个 agent（全部 `在 Codex 中仅当用户明确允许 delegation 时使用 spawn_agent 并行处理`），使用 `.claude/agents/` 中的 agent 定义（`agent role / prompt template` 参数）。

Agent 定义文件包含完整的角色 persona、搜索清单、输出格式和引用规则。dispatch prompt **只需传递上下文数据**，不要重复 agent 定义中已有的内容。

| Agent | agent role / prompt template | dispatch prompt 传递的上下文 |
|-------|---------------|---------------------------|
...
**Round 1 — 构建论点**：
单条消息并行 spawn `bull` 和 `bear`（`在 Codex 中仅当用户明确允许 delegation 时使用 spawn_agent 并行处理`，命名为 `bull` 和 `bear`），prompt 包含：
...
收齐双方 Round 1 论点后，**并行**用 `send_input` 向两个 idle agent 发送 rebuttal 指令：
...
如果 bull confidence score 与 bear risk score 差距 >= 5，或双方在核心事实上矛盾，触发 Round 3。并行 send_input 给 bull 和 bear，要求聚焦最关键的 2-3 个因素，简洁陈述（400-600 词）。
...
**4d. 写报告**：中文，保存到 `reports/{TICKER}_{DATE}.md`
